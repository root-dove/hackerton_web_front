...
const LogShow = () => {
    ...
    useEffect(() => {
        const initialLogs = Array(7).fill().map(() => generateRandomLog());
        setLogList(initialLogs);
    }, []);

    const addRandomLog = () => {
        setLogList(prevLogs => [generateRandomLog(), ...prevLogs.slice(0, 6)]);
    }

    useEffect(() => {
        const interval = setInterval(addRandomLog, 100); // 5초마다 새 로그 추가
